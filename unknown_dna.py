...
if __name__ == '__main__':
    ...
    for line in fileinput.input():
        if '>' in line:
            string_id = line.rstrip()[1:]
            strings[string_id] = {'string': '', 'gc': 0.0}
            continue

        strings[string_id]['string'] += line.rstrip()
        strings[string_id]['gc'] = gc_content(strings[string_id]['string'])

    string_id = max(strings, key=lambda key: strings[key]['gc'])

    print(string_id)
    print(f"{strings[string_id]['gc'] * 100}")
