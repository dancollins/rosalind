...
def calculate_rabbits(n_months, k_pairs):
    if n_months <= 2:
        return 1

    return (calculate_rabbits(n_months - 1, k_pairs) +
            k_pairs * calculate_rabbits(n_months - 2, k_pairs))

if __name__ == '__main__':
    for line in fileinput.input():
        n, k = line.rstrip().split()
        print(calculate_rabbits(int(n), int(k)))
